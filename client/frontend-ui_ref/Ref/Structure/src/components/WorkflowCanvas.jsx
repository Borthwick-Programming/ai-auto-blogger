...
export default function WorkflowCanvas({ projectId }) {
  ...
  const [nodes, setNodes, onNodesChange] = useNodesState([]);
   ...
   const [selectedProject, _setSelectedProject] = useState(
    () => localStorage.getItem('wf-selectedProject') || ''
  );
  const setSelectedProject = id => {
    localStorage.setItem('wf-selectedProject', id);
  ...
  setNodes(nds =>
    ...
    fetch(`/api/projects/${selectedProject}/nodeinstances/${n.id}`, {
      ...
      body: JSON.stringify({
        ...
        .then(r => {
          if (!r.ok) throw new Error(`Save node position failed ${r.status}`);
          console.log('Node position saved', n.id, n.position);
          })
        .catch(console.error);
...
const onNodesDelete = useCallback(del => {
  del.forEach(n =>
   ...
   // track clicked node for configuration
    ...
    const recalcPanel = useCallback((nodeId) => {
    ...
    setConfigPos({
     ...
     const onNodeClick = useCallback((event, node) => {
      ...
      // Whenever the nodes array changes (e.g. you dragged one), re-position the panel
  useEffect(() => {
    if (configNode) {
      recalcPanel(configNode.id);
    }
  }, [nodes, configNode, recalcPanel]);
